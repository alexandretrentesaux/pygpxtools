...
import os
import sys
from pygpxtools.__init__ import logger
    ...
    if os.path.exists(os.path.join(tempfile.gettempdir(), '.strava')):
        ...
        if config.has_section('default'):
            if config.has_option('default', 'client_id') and \
                ...
                logger.error('Missing option in strava credentials. Please configure first')
                sys.exit(-1)
        else:
            logger.error('Missing section in Strava credentials. Please configure first')
            sys.exit(-1)
    else:
        logger.error('Missing Strava credentials cache file. Please configure first')
        sys.exit(-1)
